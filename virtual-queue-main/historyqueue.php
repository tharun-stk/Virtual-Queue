...
$sqlcusthistory="select * from $customerhistory";
$sqlcusthistoryvalue=$connect->select($connection,$sqlcusthistory);
if($sqlcusthistoryvalue!=null){
$sqlcusthistoryrows=$connect->noofRows($sqlcusthistoryvalue);
}
...
<head>
    ...
    <link rel="stylesheet" href="css/historyqueue.css">
    <title>Customer History</title>
</head>
<body>
<a href="customer.php"><button class="button button2 ">My Dashboard</button></a>
...
<div class="container">
  ...
  <?php while($sqlcusthistoryvaluedata=$connect->fetchData($sqlcusthistoryvalue)){
    $sqlname=$sqlcusthistoryvaluedata['shopnumber'];
    $sqlshopname="select shopname from shopdetails where phone='$sqlname'";
    $sqlshopnamevalue=$connect->select($connection,$sqlshopname);
    $sqlshopnamevaluedata=$connect->fetchData($sqlshopnamevalue);
    $finalsqlshopname=$sqlshopnamevaluedata['shopname'];
    ?>
  
    
      
        <tr>
          <td><?php echo $sqlcusthistoryvaluedata['phone'] ?></td>
          <td><?php echo $finalsqlshopname ?></td>

          <td><?php echo $sqlcusthistoryvaluedata['tokenno'] ?> </td>
          <td><?php echo $sqlcusthistoryvaluedata['date'] ?></td>
...
if($sqlcusthistoryvalue==null) {?>
 <div class="empty-state">
  <div class="empty-state__content">
    ...
    <div class="empty-state__message">No history right now </div>
    <div class="empty-state__help">
      Please join atleast one queue and complete it.
    </div>
  </div>
</div>
