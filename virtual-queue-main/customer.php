<?php 
include 'queuefunctions.php';
  $connect= new Functions();
  $connection=$connect-> getConection();
  $customerphone=$_SESSION["phone"];
  $namesql="select * from user where phone='${customerphone}'";
  $namesqlconnect=$connect->select($connection,$namesql);
  $name=$connect->fetchdata($namesqlconnect);
  $sql="select * from shopdetails";
  $value=$connect->select($connection,$sql);
  $valuecountrows=$connect->noofRows($value);
  
  if(isset($_POST['logout'])){
    
      header('location:index.php');
  }
 
  
  if(isset($_POST['location'])){
  
   

    $_SESSION['custlatitude']=$_POST['latitude'];
    $_SESSION['custlongitude']=$_POST['longitude'];

  }
  if(isset($_POST['join'])){
     
      
    if(isset($_SESSION['custlatitude']) and isset($_SESSION['custlongitude'])){ 
        
            $queue=new Queuefunctions();
            if($name['isactive']==0){
            $joinqueue=$queue->joinqueue($_POST['join'],$customerphone,$_SESSION['custlatitude'],$_SESSION['custlongitude']);
            
            if($joinqueue==0){
                header('location:queuewaiting.php');
            }
            elseif($joinqueue==1){
               header('location:queue.php');
       
            }
            elseif($joinqueue==-1){
                ?><div class='alert'><h2>TOKEN CLOSED FOR NOW<h2></div><?php
            }
       
           }
           else{
              ?> <div class='alert'><h2>You are already in queue<h2></div><?php
           }
           
           }
        
 else
   {
       ?><div class='alert'><h2> please update your location</h2></div>
    <?php }
        
    
   }
  
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="css/customer.css">
    <title>Customer Dashboard</title>
</head>
<body onload=getLocation();>
<nav class="navigation">
        <div class="logo">
            <img src="images/Eagle Mascot Logo.jpg" alt="LOGO">

        </div>
        <p><?php echo "HI "; echo $name['name'];?><p>
            <?php 
$customerfullhistory=$customerphone.'fullhistory';
$checktable="select * from $customerfullhistory";
$checktablevalue=$connect->select($connection,$checktable);
// $checktablevaluerows=$connect->noofRows($checktablevalue);


if($checktablevalue!=null){
?>
        <a href="queuewaiting.php"><button class="button button2 ">My Queues</button></a><?php } ?>
       <form method='POST' id=myForm class='myform' action="<?php htmlspecialchars($_SERVER["PHP_SELF"]) ?>">
      
        <input type="hidden" class="lat" name="latitude" id="demo" value="">
          <input type="hidden" class="lon" name="longitude" id="longitude" value="">
          <button name='location' class="button button2 button1">Update your location</button>
        <button name='logout' class="button button2 button1">Logout</button>

</form>



</nav>
<hr>
<?php if(isset($_SESSION['custlatitude']) and isset($_SESSION['custlongitude'])){
 if($valuecountrows>0 ){?>
<div class='list'>
    <?php while ($shopdetails=$connect->fetchdata($value))
     {
       $shoplatitude=$shopdetails['latitude'];
       $shoplongitude=$shopdetails['longitude'];
       $custlatutide=$_SESSION['custlatitude'];
       $custlongitude=$_SESSION['custlongitude'];

       $distance=$connect->DistAB($custlatutide,$custlongitude,$shoplatitude,$shoplongitude);
      

       if($distance<10){
            

         if($shopdetails['isactive']==1){
             ?>
<form method='POST' class="myForm" action="<?php htmlspecialchars($_SERVER["PHP_SELF"]) ?>">

     <div class=summary>
         <div class="img"><img src=<?php echo $shopdetails['image'] ;?> alt="" srcset=""></div>
         <div class='des'><h2>Description</h2>
         <p><?php echo "Name:" ; echo $shopdetails['shopname'] ; ?></p> 

            <p><?php echo $shopdetails['descrption'] ; ?></p> 
           
            <div class=table><table>
            <tr>
                <th>
                    TOTAL TOKENS
                </th>
                <th>AVAILABLE TOKENS</th>
</tr>
<tr><td><?php echo $shopdetails['totaltokens'] ;?></td><td><?php echo $shopdetails['availabletokens'] ;?></td></tr></table></div>


<button name='join' value='<?php echo $shopdetails['phone']; $_SESSION['shopphone']=$shopdetails['phone'];?>' class='button  button2'> Join queue</button>

     </form>
     <?php } 
     
     elseif($valuecountrows==0){
          ?>
     <div class="empty-state">
  <div class="empty-state__content">
    <div class="empty-state__icon">
      <img src="data:image/png;base64," alt="">
    </div>
    <div class="empty-state__message">No shop available right now </div>
    <div class="empty-state__help">
      Please try after sometime.
    </div>
  </div>
</div>
  <?php } }
else{ ?>
  <div class="empty-state">
<div class="empty-state__content">
<div class="empty-state__icon">
<img src="data:image/png;base64," alt="">
</div>
<div class="empty-state__message">No shop available right now </div>
<div class="empty-state__help">
Please try after sometime.
</div>
</div>
</div>

<?php }}}else{ ?>
        <div class="empty-state">
  <div class="empty-state__content">
    <div class="empty-state__icon">
      <img src="data:image/png;base64," alt="">
    </div>
    <div class="empty-state__message">No shop available right now </div>
    <div class="empty-state__help">
      Please try after sometime.
    </div>
  </div>
</div>

<?php } }
else{
  ?><div class='alert'><h2> please update your location</h2></div>
<?php }
  ?>
           
     </div>
     </div>
    
       


    
    
     <script>


function getLocation() {
    getLatitude();
    getLongitude();

}

function getLatitude() {
    
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(showPositionla);
    } else {
        x.innerHTML = "Geolocation is not supported by this browser.";
    }
}

function showPositionla(position) {
    
    var z= position.coords.latitude;

    document.querySelector('.lat').value=z;
}




function getLongitude() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(showPositionlo);
    } else {
        y.innerHTML = "Geolocation is not supported by this browser.";
    }
}

function showPositionlo(position) {
   var t=position.coords.longitude;
   document.querySelector('.lon').value=t;

  

}


</script>




    
</body>
</html>