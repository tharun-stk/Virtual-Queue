<?php
include 'queuefunctions.php';
$connect= new Functions();
$connection=$connect-> getConection();
$customerphone=$_SESSION["phone"];
$namesql="select name from user where phone='${customerphone}'";
$namesqlconnect=$connect->select($connection,$namesql);
$name=$connect->fetchdata($namesqlconnect);
$customerhistory=$customerphone.'custhistory';
$customersortedhistory=$customerphone.'sortedhistory';
$selectcurentrow="select * from $customersortedhistory where phone='${customerphone}'";
$selectcurentrowvalue=$connect->select($connection,$selectcurentrow);
if($selectcurentrowvalue!=null){
$rowsincustomer=$connect->noofRows($selectcurentrowvalue);


if($rowsincustomer>0){
$selectcurentrowdata=$connect->fetchdata($selectcurentrowvalue);
$inqueueshop=$selectcurentrowdata['shopnumber'];
$getshodetails="select * from shopdetails where phone='${inqueueshop}'";
$getshopdetailsvalue=$connect->select($connection,$getshodetails);
$getshodetailsdata=$connect->fetchdata($getshopdetailsvalue);
$shopnamesortedhistory=$getshodetailsdata['shopname'].'sortedhistory';
$sqlcountlive="select * from $shopnamesortedhistory order by tokenno + 1 asc limit 1";
$sqlcountlivevalue=$connect->select($connection,$sqlcountlive);
$sqlcountlivedata=$connect->fetchData($sqlcountlivevalue);
}}


if(isset($_POST['quit'])){
  $selectcurentrow="select * from $customersortedhistory where phone='${customerphone}'";
  $selectcurentrowvalue=$connect->select($connection,$selectcurentrow);
    $selectcurentrowdataphone=$connect->fetchData($selectcurentrowvalue);
    $inqueueshop=$selectcurentrowdataphone['shopnumber'];
  $queue=new Queuefunctions();
  $rowremove=$queue->exitqueue($customerphone,$inqueueshop);
  if($rowremove==1){
  header('location:queue.php');
  }

}







?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="css/queue.css">
    <title>Queue</title>
</head>
<body>
<nav class="navigation">
        <div class="logo">
            <img src="images/Eagle Mascot Logo.jpg" alt="LOGO">

        </div>
        <p><?php echo "HI "; echo $name['name'];?><p>
        <a href="historyqueue.php"><button class="button button2 ">History of Queues</button></a>
        <a href="customer.php"><button class="button button2 ">Dashboard</button></a>

      



</nav>
<?php 
if($selectcurentrowvalue!=null){

if($rowsincustomer>0){ ?>

    
<form method='POST' class="myForm" action="<?php htmlspecialchars($_SERVER["PHP_SELF"]) ?>">
    
    <div class='head'><h2>Present Queue</h2></div>
        
        <div class=summary>
         
         <div class="img"><img src=<?php echo $getshodetailsdata['image'] ;?> alt="" srcset=""></div>
         <div class='des'><h2>Description</h2>
         <p><?php echo "Name:" ; echo $getshodetailsdata['shopname'] ; ?></p> 

            <p><?php echo $getshodetailsdata['descrption'] ; ?></p>
            <div class='division'>
         <div class='divide'> 
            <div class='live'> <p>Live Token</p>
            <p><?php echo $sqlcountlivedata['tokenno'];?><p>
           

</div>

       <div class='divide'>  <div class='live'> <p>Your Token</p>
            <p><?php echo $selectcurentrowdata['tokenno'];?><p></div>
           
</div>
</div>
<div class='drop' > 
  <button name ='quit' class="button button2 ">Quit Queue</button>
</div>



</div>
</form>
<?php }
else{
  ?>
  <div class="empty-state">
  <div class="empty-state__content">
    <div class="empty-state__icon">
      <img src="data:image/png;base64," alt="">
    </div>
    <div class="empty-state__message">No queue has been joined.</div>
    <div class="empty-state__help">
     Check your history here.
    </div>
  </div>
</div>


<?php }}
else{
  ?>
  <div class="empty-state">
  <div class="empty-state__content">
    <div class="empty-state__icon">
      <img src="data:image/png;base64," alt="">
    </div>
    <div class="empty-state__message">No queue has been joined.</div>
    <div class="empty-state__help">
     Check your history here.
    </div>
  </div>
</div>


<?php }?>
<script src="https://code.jquery.com/jquery-3.3.1.min.js">
  </script>
   
  <script type="text/javascript">
    $(document).ready(function () {
      setTimeout(function () {
      
        location.reload(true);
      }, 15000);
    });
    </script>
            
</body>
</html>